...
def bmn_loss_func(pred_bm, pred_start, pred_end, gt_iou_map, gt_start, gt_end, bm_mask):
    ...
    #cls_loss = top_ce_loss(gt_cls, pred_cls)
    loss = tem_loss + 10 * pem_reg_loss + 10 * pem_cls_loss
...
def ce_loss(gt_cls, pred_cls):
    ce_loss = F.cross_entropy(pred_cls, gt_cls, reduction='none')
    return ce_loss